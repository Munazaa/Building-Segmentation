...
def load_image_gt(dataset, config, image_id, augmentation=None,
                  use_mini_mask=True,resize=True):
    """Load and return ground truth data for an image (image, mask, bounding boxes).
    ...
    # Resize masks to smaller size to reduce memory usage
    if use_mini_mask and config.USE_MINI_MASK:
        mask = utils.minimize_mask(bbox, mask, config.MINI_MASK_SHAPE)
...
class MaskRCNN(object):
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
              augmentation=None, custom_callbacks=None, no_augmentation_sources=None,workers=False):
        """Train the model.
