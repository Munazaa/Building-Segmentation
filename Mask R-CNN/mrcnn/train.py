...
Original file is located at
    https://colab.research.google.com/drive/1VS_Rjz8PPWjv8oYetuh7hF4xix1_nxEv

# **Training Notebook**

> In this notebook we will be focusing on the training of our  model.
...
##Load Configurations for Training
> We now load the configurations for training the model.
"""

config= MappingTrainingConfig()
config.display()

"""##Create Model to Train


> Now we will create our model in train state. We would do this by using the configurations that we loaded in the previous cell.
...
path=MODEL_DIR_WEIGHTS+'/TransferableWeights.h5'
model.load_weights(path, by_name=True,
...
augmentation =iaa.Sometimes(5/6,iaa.Sequential([
                    iaa.Affine(scale=(1, 1.5),cval=(0, 1)),
                    iaa.SomeOf((1, 5),[
                    ...
                    # iaa.Multiply((0.5, 2.0), name="Multiply"),
                    # iaa.Affine(translate_percent={"x": (-0.1, 0.1), "y": (-0.1, 0.1)},cval=(0, 1)),
                    # iaa.Affine(rotate=(-5, 5),cval=(0, 1)),  # rotate by -45 to 45 degrees
                    # iaa.Affine(shear=(-10, 10),cval=(0, 1)),
...
for id in image_ids:
  ...
  for i in range(10):
    ...
    msk = det.augment_image(mask.astype(np.uint8),
                                 hooks=ia.HooksImages(activator=hook))
    msk = msk.astype(np.bool)
    visualize.display_top_masks(img, msk, class_ids, dataset.class_names)

"""## Train the head Layers of Model Only


> Now we train head part of our model (Input and output layers only). We also choose a high learning rate here. We do this to help the model converge in short time as it is start of the training process and model will not overshoot as much as it would once it gets close to optimal point. This also helps the model to learn most of the major features that differentiate each class from each other as fast as possible. we can train the model again later to fine tune all the parameters to get as close to optimal point as possible.
...
#             epochs=5,
#             layers='heads')
# pltGraph(range(5),model.keras_model.history.history)

# model.train(dataset_train, dataset_val,
#             learning_rate=config.LEARNING_RATE,
#             epochs=10,
#             layers='heads')
# pltGraph(range(10),model.keras_model.history.history)

# model.train(dataset_train, dataset_val,
#             learning_rate=config.LEARNING_RATE,
#             epochs=20,
#             layers='heads')
# pltGraph(range(20),model.keras_model.history.history)
...
model.train(dataset_train, dataset_val,
            ...
            epochs=15,
            layers='heads')
pltGraph(range(15),model.keras_model.history.history)
...
path=MODEL_DIR_WEIGHTS+'/ModelWeights.h5'
model.load_weights(path, by_name=True)
model.train(dataset_train, dataset_val,
            learning_rate=config.LEARNING_RATE,
            epochs=15,
...
Now we have trained the model's head layers and have learned few major features that diffrentiates each category from each other. Now we train all the internal deep layers of our model in order to learn even the smallest features that might help it to distinguish between each class. We also choose a very small learning rate in order to avoid the model to overshoot from the optimal point and we slowly train our model to get to the optimal point.
"""
...
pltGraph(range(50),model.keras_model.history.history)
...
pltGraph(range(25),model.keras_model.history.history)
...
augmentation =iaa.Sometimes(5/6,iaa.Sequential([
                    iaa.Affine(scale=(1, 1.5),cval=(0, 0)),
                    iaa.SomeOf((1, 2),[
                    # iaa.AdditiveGaussianNoise(scale=0.05 * 255, name="AWGN"),
                    # iaa.GaussianBlur(sigma=(0.0, 2.0), name="Blur"),
                    # iaa.Dropout([0.0, 0.05], name='Dropout'), # drop 0-5% of all pixels
